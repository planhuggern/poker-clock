...
function TerritoryActions({ territory, territoryState, currentPlayer, dispatchGameAction }: TerritoryActionsProps) {
...
function MissionCard({ setMissionRevealed }: { setMissionRevealed: (v: boolean) => void }) {
  if (isMvpGame()) return null;
  const myPlayer = state.gameState!.players.find((p) => p.id === state.myPlayerId);
  if (!myPlayer) return null;

  const mission = MISSIONS.find((m) => m.id === myPlayer.mission);
  const target = myPlayer.target ? state.gameState!.players.find((p) => p.id === myPlayer.target) : null;
...
function RentModalView({ modal, clearModal, dispatchGameAction }: { modal: RentModal; clearModal: () => void; dispatchGameAction: (a: Action) => void }) {
  return (
