...
const PIECE_POS: Record<string, [number, number]> = Object.fromEntries(
  Object.entries(mapData._rawTerritories).map(([id, pts]) => [
    id,
    polygonCentroid(pts as [number, number][]),
  ]),
);
...
function buildTooltip(draw: Svg): TooltipWidget {
  const group = draw
    .group()
    .attr({ id: "map-tooltip-svg", "pointer-events": "none" })
    .hide();
  ...
  const owner = group
    .text("")
    .attr({ x: 10, y: 45, fill: "#888", "font-size": 10 });
...
function setTooltipText(
  ...
  const district =
    territory.type === "territory" ? DISTRICTS[territory.district] : null;
  tooltip.title.text(territory.name);
  tooltip.district.text(
    territory.type === "checkpoint"
      ? "Checkpoint · friområde"
      : (district?.name ?? ""),
  );
  tooltip.owner.text(
    territory.type === "checkpoint"
...
export function createMapAdapter(
  ...
  if (mapData.specialShapes.oslofjorden) {
    ...
    draw
      .path(mapData.specialShapes.oslofjorden)
      .fill("url(#water-pat)")
      .opacity(0.5);
  ...
  for (const [districtId, pathD] of Object.entries(mapData.districtShapes)) {
    ...
    if (rawPts) {
      ...
      districtLabelData.push({
        text: districtInfo?.name?.split(" ")[0] ?? districtId,
        x: lx,
        y: ly,
      });
    }
  ...
  for (const [checkpointId, checkpoint] of Object.entries(CHECKPOINTS)) {
    ...
    group
      .polygon(
        `${cx},${cy - 18} ${cx + 13},${cy} ${cx},${cy + 18} ${cx - 13},${cy}`,
      )
      .attr({
    ...
    group
      ...
      .attr({ class: "checkpoint-label", x: cx, y: cy + 28 });
    ...
    group.on("mouseenter", (event) => {
      ...
      tooltip.group.show();
      moveTooltip(event as PointerEvent);
    });
  ...
  for (const territory of TERRITORIES) {
    ...
    const units = group
      .text(String(terr.neutralUnits))
      .attr({ class: "terr-units", id: `units-${terr.id}`, x: cx, y: cy + 8 });

    group.on("mouseenter", (event) => {
      setTooltipText(tooltip, terr, currentGameState);
      tooltip.group.show();
      moveTooltip(event as PointerEvent);
    });
    group.on("mousemove", (event) => moveTooltip(event as PointerEvent));
  ...
  const borderGroup = draw
    .group()
    .attr({ id: "district-border-layer", "pointer-events": "none" });
  for (const pathD of Object.values(mapData.districtShapes)) {
    borderGroup
      .path(pathD)
      .attr({ fill: "none", stroke: "#3a4a3a", "stroke-width": 1.8 });
  }

  const districtLabelGroup = draw
    .group()
    .attr({ id: "district-label-layer", "pointer-events": "none" });
  for (const { text, x, y } of districtLabelData) {
    districtLabelGroup.text(text).attr({ class: "district-label", x, y });
  }

  const pieceLayer = draw
    .group()
    .attr({ id: "piece-layer", "pointer-events": "none" });
  ...
  function update({
    ...
    for (const districtId of Object.keys(DISTRICTS)) {
      const dTerrs = TERRITORIES.filter(
        (t): t is Territory =>
    ...
    for (const territory of TERRITORIES) {
      ...
      if (owner) {
        ...
        nodes.poly.attr({
          ...
          "fill-opacity": 0.85,
          stroke: owner.color,
          filter: fullDistrict ? "url(#district-glow)" : "",
        });
      } else {
        const districtColor =
          DISTRICTS[(territory as Territory).district]?.color ?? "#1a1a2a";
        nodes.poly.attr({
          fill: districtColor,
          "fill-opacity": 0.25,
          stroke: "rgba(201,168,76,0.5)",
    ...
    for (const player of currentGameState.players ?? []) {
      ...
      let coords: [number, number] | undefined =
        PIECE_POS[position] ?? TERRITORY_POS[position];
