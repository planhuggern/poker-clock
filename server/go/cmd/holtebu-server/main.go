...
func main() {
	...
	http.HandleFunc(
		...
		methodDispatch(map[string]http.HandlerFunc{
			http.MethodGet:  pokerclock.ListTournaments(db),
			http.MethodPost: pokerclock.CreateTournament(db),
		}),
	)
	http.HandleFunc(
		tournamentPath+"/",
		pokerclock.GetTournamentByID(db, tournamentPath+"/"),
	)
